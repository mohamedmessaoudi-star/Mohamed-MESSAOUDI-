...
type RoleKey = "cad" | "struct" | "bimlo";

interface Check { id: number; text: string; type: string; done: boolean; }
interface Finding { type: string; msg: string; }
interface RoleState {
  checks: Check[];
  aiResults: Finding[];
  aiScore: number;
  aiRun: boolean;
  validated: boolean;
  sentToStruct?: boolean;
  sentToBimlo?: boolean;
}
interface AppState { cad: RoleState; struct: RoleState; bimlo: RoleState; }
...
export default function App() {
  ...
  const [scanning, setScanning] = useState<Record<RoleKey, boolean>>({ cad: false, struct: false, bimlo: false });
  ...
  const runAI = async (role: RoleKey) => {
    ...
    try {
      ...
      const results: Finding[] = JSON.parse(text.replace(/```json|```/g, "").trim());
      const score = Math.min(99, results.reduce((acc, f) => acc + (f.type === "ok" ? 20 : f.type === "warn" ? 8 : f.type === "info" ? 5 : 0), 0));
      setScanning(s => ({ ...s, [role]: false }));
      setState(s => ({ ...s, [role]: { ...s[role], aiResults: results, aiScore: score, aiRun: true } }));
      showNotif(`🤖 IA ${ROLES[role].name} — Analyse terminée · Score ${score}%`);
    } catch (err) {
      setScanning(s => ({ ...s, [role]: false }));
  ...
  const totalDone = (Object.keys(state) as RoleKey[]).reduce((a, k) => a + state[k].checks.filter(c => c.done).length, 0);
  const totalItems = Object.values(ROLES).reduce((a, r) => a + r.checks.length, 0);
  const aiDone = (["cad", "struct", "bimlo"] as RoleKey[]).filter(k => state[k].aiRun).length;
  const valDone = (["cad", "struct", "bimlo"] as RoleKey[]).filter(k => state[k].validated).length;

  const RoleCard = ({ role, cardClass }: { role: RoleKey; cardClass: string }) => {
    const R = ROLES[role];
    const r = state[role];
    const colors: Record<RoleKey, string> = { cad: "#f59e0b", struct: "#a78bfa", bimlo: "#00c8ff" };
      ...
      <div className={`card ${cardClass}`} style={isLocked ? { opacity: 0.4, pointerEvents: "none" } : {}}>
        ...
        <div className="card-body">
          ...
          <div className="section-title">
            ...
            <span style={{ fontSize: 10, color: "#374151", fontFamily: "monospace", marginLeft: 8 }}>{r.checks.filter(c => c.done).length}/{r.checks.length}</span>
          </div>
          <div className="checklist">
            {r.checks.map(c => (
              <div key={c.id} className={`check-item ${c.done ? "checked" : ""}`} onClick={() => toggleCheck(role, c.id)}>
                ...
                <span className={`check-badge badge-${c.type}`}>{c.type === "human" ? "PERSO" : c.type === "cross" ? "CROISÉ" : "FINAL"}</span>
              </div>
            ))}
          </div>
          ...
          <div className="action-bar">
            <button className={`btn btn-primary-${role}`} onClick={() => !isScanning && !r.aiRun && !r.validated && runAI(role)} disabled={isScanning || r.aiRun || r.validated}>
              {isScanning ? "⟳ Claude analyse…" : r.aiRun ? "✓ IA Complète" : "▶ Lancer IA Claude"}
            </button>
            <button className="btn btn-outline" onClick={() => canValidate && validate(role)} disabled={!canValidate} style={canValidate ? { borderColor: color, color } : {}}>
      ...
      <div className="app">
        <div className="header">
          ...
          <div className="header-sub">
            Plans Fabrication PDF · Note de Calcul · IFC · Intégration Paysagère · <span style={{ color: "#00c8ff" }}>IA Claude Réelle ✓</span>
          </div>
        </div>
        ...
        <div className="pipeline">
          <div className="pipeline-step">
            <div className="pipeline-node">
              <div className={`pipeline-badge cad ${!state.cad.validated ? "active" : ""}`}>W</div>
              <div className="pipeline-label"><strong>WALA</strong>Ing. CAD</div>
            </div>
          </div>
          ...
          <div className="pipeline-step">
            <div className="pipeline-node">
              ...
              <div className="pipeline-label">
                <strong style={{ color: state.bimlo.validated ? "#10b981" : undefined }}>{state.bimlo.validated ? "ARCHIVÉ" : "ARCHIVE"}</strong>BIMLO
              </div>
            </div>
          </div>
        </div>
